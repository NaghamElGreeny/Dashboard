...
export default function MainDataReturnPolicy({ isLoading }: { isLoading?: boolean }) {
    const { t } = useTranslation();
    const { values } = useFormikContext<{ [key: string]: any }>();

    const fields = [
        { name: 'return_policy_ar', label: 'labels.return_policy_ar' },
        { name: 'return_policy_en', label: 'labels.return_policy_en' },
    ];
