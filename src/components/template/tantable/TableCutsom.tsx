...
const TableCompCustom = ({
    ...
    const table = useMantineReactTable({
        ...
        // handle select the row in table view
        mantineTableBodyRowProps: ({ row }) => ({
            onClick: row.getToggleSelectedHandler(),
            sx: { cursor: isEnableRowSelect ? 'pointer' : '' },
        }),
    ...
    useEffect(() => {
        ...
        const initialSelection = Object.fromEntries(selectedRowIds.map((id: any) => [id, true]));

        if (JSON.stringify(rowSelection) !== JSON.stringify(initialSelection)) {
    ...
    useEffect(() => {
        if (!isEnableRowSelect || !setFieldValue) return;

        const selectedRows = table.getSelectedRowModel().rows.map((row) => ({
            product_detail_id: row.original.id,

            // [`products[${index}][product_detail_id]`]: row.original.id,
            // [`${name}[${index}][id]`]: row.original.id,
        }));

        if (JSON.stringify(selectedRows) !== JSON.stringify(previousSelectedRows)) {
