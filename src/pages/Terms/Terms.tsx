...
import type { FetchTermsData, Terms } from './types';
...
export default function Terms() {
    ...
    const [termId, setTermId] = useState<Object>('');
    const [page, setPage] = useState(1);
    const [opened, setOpen] = useState<boolean>(false);
    const [selectedDescription, setSelectedDescription] = useState<string>('');

    const columns: MRT_ColumnDef<Terms>[] = [
        {
            header: '#',
        ...
        ...(hasPermission('update-Term') || hasPermission('destroy-Term')
            ? [
                  {
                      ...
                      Cell: ({ renderedCellValue, row }: any) => (
                          ...
                          >
                              {hasPermission('update-Term') && (
                                  <Link
                                      to={`/public-pages/terms/edit/${row.original?.id}`}
    ...
    const { mutate: Delete } = useMutate({
        mutationKey: [`term/delete/${termId}`],
        endpoint: `term/delete/${termId}`,
        ...
        onSuccess: async (data: any) => {
            ShowAlertMixin({
                ...
                title:
                    data?.data?.message ||
                    t('isDeletedSuccessfully', { name: t('breadcrumb.terms.title') }),
            });
    ...
    const endpoint = `term/index?${searchParams.toString()}`;

    const {
        data: terms,
        refetch,
        isLoading,
    } = useFetch<FetchTermsData>({
        endpoint: endpoint,
        queryKey: [endpoint],
    });
    ...
    return (
        <>
            ...
            <TableCompCustom
                ...
                title={t('breadcrumb.terms.add')}
                page={page}
                setPage={setPage}
                isLoading={isLoading}
                downloadAndExport={
                    <>
                        {hasPermission('store-Term') && (
                            <Link
                                to="/public-pages/terms/add"
